...
class SecondPoint:
    ...
    def validate_inputs(self):
        if self.type_value.get().strip() and str(self.amplitude_value.get()).strip() and str(self.analogFrequency_value.get()).strip() and str(self.samplingFrequency_value.get()).strip() and str(self.phaseShift_value.get()).strip():
            return True
        else:
